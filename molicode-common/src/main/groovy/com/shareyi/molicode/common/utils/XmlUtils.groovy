...
class XmlUtils {
    ...
    static AutoMakeVo getAutoMakeByContent(String xmlContent) {
        ...
        List<CustomFieldGroup> customFieldGroupList = [];
        autoMake.customFieldGroupList = customFieldGroupList;
        def customFieldGroupSettingNodes = autoMakeXml.customFieldGroupSetting;
        if (customFieldGroupSettingNodes != null && customFieldGroupSettingNodes.size() > 0) {
            autoMakeXml.customFieldGroupSetting.customFieldGroup.each { customFieldGroupNode ->
                CustomFieldGroup customFieldGroup = new CustomFieldGroup();
                customFieldGroup.groupKey = getStringFromNode(customFieldGroupNode, "groupKey", true)
                customFieldGroup.desc = getStringFromNode(customFieldGroupNode, "desc", true);
                customFieldGroup.configShow = getBooleanFromNode(customFieldGroupNode, "configShow", true);
                customFieldGroup.groupExpression = getStringFromNode(customFieldGroupNode, "groupExpression", true);
                customFieldGroupList.add customFieldGroup;
            }

        }
        if (CollectionUtils.isEmpty(customFieldGroupList)) {
            customFieldGroupList.addAll(MoliCodeConstant.DEFAULT_CUSTOM_FIELD_GROUP_LIST);
        }
     ...
     * @return
     */
    static String getBooleanFromNode(Node node, String key, boolean defVal) {
        String strVal = getStringFromNode(node, key, true);

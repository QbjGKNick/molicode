    ...
    >
      <Row>
        <Col span="24">
          <Form-item label="代码输出方式" prop="outputType" style="width: 95%">
            <dict-radio
              v-model="formItems.outputType"
              :disabled="disableInput"
              :kind="this.constants.dicts.dictKinds.OUTPUT_TYPE_DICT"
            ></dict-radio>
          </Form-item>
        </Col>
        ...
        <Col span="24" v-show="formItems.outputType === '1'">
          <Form-item
            label="代码输出根目录"
            prop="projectOutputDir"
            style="width: 95%"
          >
            <file-chooser
              v-model="formItems.projectOutputDir"
              :disabled="disableInput"
      ...
      <Row>
        <Col span="24">
          <Form-item label="模板类型" prop="templateType" style="width: 95%">
            <dict-radio
              v-model="formItems.templateType"
              :disabled="disableInput"
              :kind="this.constants.dicts.dictKinds.TEMPLATE_TYPE_DICT"
            ></dict-radio>
          </Form-item>
        </Col>
      </Row>
      ...
      <div v-show="formItems.templateType == 'git'" style="margin-bottom: 20px">
        <Form-item label="git url" prop="gitUrl" style="width: 95%">
          <Input
            v-model="formItems.gitUrl"
            :maxlength="200"
            :disabled="disableInput"
          >
            <Button slot="append" icon="ios-search" @click="repoChoose"
              >选择</Button
            >
          </Input>

          <git-repo-list
            @itemChoose="chooseGitRepo"
        ...
        <Form-item
          label="模板相对路径"
          prop="templateRelativePath"
          style="width: 95%"
        >
          <Input
            v-model="formItems.templateRelativePath"
            :maxlength="200"
            :disabled="disableInput"
          ></Input>
        </Form-item>
      ...
      <div
        v-show="
          formItems.templateType == 'local' || formItems.templateType == null
        "
      >
        <Row>
          <Col span="24">
            <Form-item
              label="模板根目录"
              prop="templateBaseDir"
              style="width: 95%"
            >
              <file-chooser
                v-model="formItems.templateBaseDir"
                :disabled="disableInput"
                changeCurPath="1"
                dialogType="directory"
              ></file-chooser>
      ...
      <FormItem>
        ...
        <Button
          ...
          @click="fetchGitRepo"
          :loading="loading"
          v-show="formItems.templateType == 'git'"
        >
  ...
  methods: {
    save: function() {
      ...
      this.$refs["formItems"].validate(valid => {
        ...
        requestUtils.postSubmit(
          this,
          constants.urls.conf.acConfig.save,
          this.getConfigData(),
          function(data) {
    ...
    fetchGitRepo() {
      this.$refs["formItems"].validate(valid => {
        ...
        requestUtils.postSubmit(
          this,
          constants.urls.repo.git.fetchRepo,
          gitRepoParam,
          function(data) {
            this.$Message.success({
    ...
    _outputTypeChange: function() {
      this.formRules.projectOutputDir[0].required =

...
class TableModelDatabaseHandler extends SimpleHandler<TableModelContext> implements TableModelHandlerAware {
    ...
    void doHandle(TableModelContext tableModelContext) {
        ...
        try {
            ...
            def tableInfo = meta.getTables(dataBaseName, null, tableModelPageVo.getTableName(), schemaTypes);
            if (tableInfo.next()) {
                String tableName = tableInfo.getString("TABLE_NAME");
                tableDefine.id = tableName;
                if (camelNameConvert) {
            ...
            def cols = meta.getColumns(dataBaseName, null, tableModelPageVo.getTableName(), null);
            while (cols.next()) {
